...
function create(options) {
  ...
  function handler(req, res, callback) {
    if (req.headers['x-github-event']) {
      handlerGithub(req, res, callback);
    }
    if (req.headers['x-gogs-event']) {
      handlerGogs(req, res, callback);
    }
     if (req.headers['x-gitlab-event']) {
      handlerGitLap(req, res, callback);
    }

  }

  function handlerGithub(req, res, callback) {
  ...
  function handlerGitLap(req, res, callback){
    console.log('on gitlap handler');
    if (req.url.split('?').shift() !== options.path)
      return callback();
